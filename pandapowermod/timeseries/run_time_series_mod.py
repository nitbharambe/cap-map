...
def run_loop(net, ts_variables, run_control_fct=run_control, output_writer_fct=_call_output_writer, **kwargs):
    ...
    for i, time_step in enumerate(ts_variables["time_steps"]):
        ...
        chk = run_time_step(net, time_step, ts_variables, run_control_fct, output_writer_fct, **kwargs)
        if chk:
            return time_step
    return chk


def run_timeseries_mod(net, time_steps=None, continue_on_divergence=False, verbose=True, **kwargs):
    """
    ...
    **Controllers**. Optionally other functions than the pp power flow can be called by setting the run function in kwargs
    ...
    violation_at_step=run_loop(net, ts_variables, **kwargs)
    return violation_at_step


def chk_violations(net):
    """
    Not original Pandapower function
    ...
    OUTPUT
        check (bool) - 'False' for no violations. 'True' for violations present

    """
    check = any(np.where(net.res_bus.vm_pu > net.bus['max_vm_pu'], True, False))
    check = check or any(np.where(net.res_bus.vm_pu < net.bus['min_vm_pu'], True, False))
    check = check or any(np.where(net.res_line.loading_percent > net.line['max_loading_percent'], True, False))
    check = check or any(np.where(net.res_trafo.loading_percent > net.trafo['max_loading_percent'], True, False))
    return check
